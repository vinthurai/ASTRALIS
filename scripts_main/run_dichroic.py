...
# -*- coding: utf-8 -*-
"""
Main script to run model for dichroic
"""
...
nominal_r,nominal_t, nominal_ghost = spectral_analysis(wl,n_front,n_substrate,n_bbar,phys_l_front,phys_l_substrate,phys_l_bbar,theta,polar)
...
ax.legend()
ax.grid()
fig.savefig(f"{OutputFilePath}/nominal_spectrum.png",dpi=600)
...
ax[0].set_ylabel('R [%]')
ax[1].set_ylabel('T [%]')
fig.savefig(f"{OutputFilePath}/contamination_spectrum.png",dpi=600)
...
df_front.to_csv(f"{OutputFilePath}/rand_error_thicknesses_nsample_"+str(n_sample)+".csv",sep=',',index=False)
df = pd.DataFrame(data=rand_dep_delta_t)
df.to_csv(f"{OutputFilePath}/rand_error_delta_t_nsample_"+str(n_sample)+".csv",sep=',',index=False)

# Read data from random thickness error files
n_sample = syst_param['general']['n_sample']['value'] 
filename = f"{OutputFilePath}/rand_error_delta_t_nsample_"+str(n_sample)+".csv"
...
contoured_2d_hist(wl,rand_dep_delta_t,OutputFilePath)
...
sys_dep_minus_r,sys_dep_minus_t,sys_dep_minus_ghost,phys_l_front_sys_minus,phys_l_rear_sys_minus = systematic_thickness_error(wl,n_front,n_substrate,n_bbar,phys_l_front,phys_l_substrate,phys_l_bbar,theta,polar,sys_dep_error,systematic_type)

# Generate a systematic error of +error% in all layers & resulting change in performance
systematic_type = 'surplus'
# Find spectral performance
sys_dep_plus_r,sys_dep_plus_t,sys_dep_plus_ghost,phys_l_front_sys_plus,phys_l_rear_sys_plus = systematic_thickness_error(wl,n_front,n_substrate,n_bbar,phys_l_front,phys_l_substrate,phys_l_bbar,theta,polar,sys_dep_error,systematic_type)
...
df.to_csv(f"{OutputFilePath}/sys_minus_rand_error_delta_t_nsample_"+str(n_sample)+".csv",sep=',',index=False)
df = pd.DataFrame(data=sys_dep_plus_rand_delta_t)
df.to_csv(f"{OutputFilePath}/sys_plus_rand_error_delta_t_nsample_"+str(n_sample)+".csv",sep=',',index=False)

# Read data
filename = f"{OutputFilePath}/sys_minus_rand_error_delta_t_nsample_"+str(n_sample)+".csv"
df_read = pd.read_csv(filename,delimiter=',')
sys_dep_minus_rand_delta_t = df_read.values
filename = f"{OutputFilePath}/sys_plus_rand_error_delta_t_nsample_"+str(n_sample)+".csv"
df_read = pd.read_csv(filename,delimiter=',')
sys_dep_plus_rand_delta_t = df_read.values

# Plot a contoured 2D histogram of the change in transmittance for Case A & B
double_contoured_2d_hist(wl,sys_dep_minus_rand_delta_t,sys_dep_plus_rand_delta_t,OutputFilePath)
...
#Calculating the nominal spectral performance 
nominal_r_temp,nominal_t_temp, nominal_ghost_temp = spectral_analysis(wl,n_front_temp,n_substrate_temp,n_bbar_temp,phys_l_front,phys_l_substrate,phys_l_bbar,theta,polar)
...
fig.savefig(f"{OutputFilePath}/sys_performances.png",dpi=600)
...
fig.savefig(f"{OutputFilePath}/ghosts.png",dpi=600)
...
for sys_dep_error_val in sys_dep_error_list:
    # Find spectral performance
    sys_dep_plus_r,sys_dep_plus_t,sys_dep_plus_ghost,phys_l_front_sys_plus,phys_l_rear_sys_plus = systematic_thickness_error(wl,n_front,n_substrate,n_bbar,phys_l_front,phys_l_substrate,phys_l_bbar,theta,polar,sys_dep_error_val,systematic_type)
...
fig.tight_layout()
fig.savefig(f"{OutputFilePath}/transwl_shifts.png",dpi=600)
